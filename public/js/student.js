...
document.addEventListener('DOMContentLoaded', (e) => {
  ...
  const StudentCategorySelect = document.getElementById('category');

  let posts;

  // Function to grab posts from the database
  const getStudent = (category) => {
    ...
    fetch(`/api/student/${categoryString}`, {
      method: 'GET',
      headers: {
  ...
  const deleteStudent = (id) => {
    ...
    }).then(() => getStudent(StudentCategorySelect.value));
  };
